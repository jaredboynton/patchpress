...
Use the EXP-01 + EXP-03 stack as the current holistic baseline:
...
7. Evidence capsules are emitted as verified sidecar metadata.
...
| Token footprint | EXP-03 default-tail result is 21,281 estimated tokens versus baseline 21,322. |
| Evidence grounding | Current replay preserves 50 evidence capsules and 41 cited source lines. |
...
- EXP-04 full evidence capsules with char ranges and typed code capsules.
...
Select EXP-01 + EXP-03 as the current best holistic implementation. It is the
only tested stack that improves state safety and user-intent retention while
holding the compact transcript footprint flat.
