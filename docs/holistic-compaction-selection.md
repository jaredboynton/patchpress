...
| EXP-08 sentinel renderer/body compression | Accepted opt-in | 5,353 no-tail replay | Reduces dry-run request body from 601,526 to 468,748 bytes, omits 137,749 chars from old tool output (~34,437 char/4 tokens), removes the duplicate current-work wrapper, and leads with model-ordered current state. Using the prior live stripped Codex byte/token ratio projects about 131,087 input tokens versus 168,325 observed stripped input tokens. | Keep `stripped` as default until a live provider run passes the scorecard; projected Sentinel input tokens are not a live compaction measurement. |
