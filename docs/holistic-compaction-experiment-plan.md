...
| EXP-09 | Add Codex-backed semantic judge path and document why provider-native opaque compaction is not applicable to cross-provider Claude handoff. | P1 cross-provider portability, semantic sufficiency | `scripts/judge-compaction-result.mjs`, docs | Judge dry-run, saved-output schema/evidence-ref validation, and live Codex `gpt-5.5` medium-reasoning validation after deterministic scorecard | Commit judge implementation and documentation update. | Judge remains advisory; deterministic gates stay authoritative; native opaque blobs are not treated as portable handoff artifacts. |
