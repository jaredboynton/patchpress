...
| EXP-09 | Add semantic judge scaffolding and document why provider-native opaque compaction is not applicable to cross-provider Claude handoff. | P1 cross-provider portability, semantic sufficiency | `scripts/judge-compaction-result.mjs`, docs | Judge dry-run plus saved-output schema/evidence-ref validation after deterministic scorecard | Commit judge scaffold and documentation update. | Judge remains advisory; deterministic gates stay authoritative; native opaque blobs are not treated as portable handoff artifacts. |
